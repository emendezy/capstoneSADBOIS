...
parseline_return parseline(const char *cmdline,
    ...
    // FIXME : those are magic values that should be #define'd
    ...
    /* $begin noexec */
    } else if ((strcmp(token->argv[0], "launch")) == 0) {
        token->builtin = BUILTIN_EXEC;
    } else if ((strcmp(token->argv[0], "map")) == 0) {
        token->builtin = BUILTIN_MAP;
    } else if ((strcmp(token->argv[0], "unmap")) == 0) {
        token->builtin = BUILTIN_UNMAP;
    /* $end noexec */
    } else {
...
static void check_blocked(void) {
    if (!init) {
        Sio_fprintf(STDERR_FILENO, "WARNING: failed to call init_job_list\n");
    }
    if (!check_block) {
        return;
    }
...
static void require_job_exists(char *func, jid_t jid) {
    if (!job_exists(jid)) {
        sio_fprintf(STDERR_FILENO,
            "FATAL: %s: invalid JID argument %d\n", func, jid);
        sio_fprintf(STDERR_FILENO,
            "This means you have a bug in your code, and you need to fix "
            "it.\n");
        abort();
    }
}

static void require_valid_state(char *func, jid_t jid, job_state state) {
    if (state != FG && state != BG && state != ST) {
        sio_fprintf(STDERR_FILENO,
            "FATAL: job_set_state: invalid state argument: %d\n", state);
        sio_fprintf(STDERR_FILENO,
            "This means you have a bug in your code, and you need to fix "
            "it.\n");
        abort();
    ...
    if (state == FG && fg_job() != 0) {
        sio_fprintf(STDERR_FILENO,
            "FATAL: job_set_state: cannot make %d the foreground job "
            "when another job %d is also the foreground job", jid, fg_job());
        sio_fprintf(STDERR_FILENO,
            "This means you have a bug in your code, and you need to fix "
            "it.\n");
        abort();
...
jid_t add_job(pid_t pid, job_state state, const char *cmdline) {
    ...
    if (verbose) {
        fprintf(stderr, "add_job: Added job [%d] %d %s\n",
            (int) job->jid, (int) job->pid, job->cmdline);
    }
...
jid_t fg_job(void) {
    ...
    if (verbose) {
        Sio_fprintf(STDERR_FILENO,
            "fg_job: No foreground job found\n");
    }
    return 0;
}
...
bool list_jobs(int output_fd) {
    ...
    for (jid_t jid = 1; jid <= MAXJOBS; jid++) {
        ...
        if (res < 0) {
            Sio_fprintf(STDERR_FILENO,
                "list_jobs: Error writing to output_fd: %d\n",
                output_fd);
            return false;
        }
    }
